...
Event OnTimerGameTime(int aiTimerID)
	trace(self, "OnTimerGameTime [" + aiTimerID + "]")
...
Event OnHit(ObjectReference RemoteSource, ObjectReference akAggressor, Form akSource, Projectile akProjectile, bool abPowerAttack, bool abSneakAttack, bool abBashAttack, bool abHitBlocked, string asMaterialName)
	...
	if RemoteSource == playerRef
		HandleCombatMessageToSituationAwareness((akAggressor as Actor))
	else
		Actor npc = (RemoteSource as Actor)
		if (pCompanion1 && pCompanion1.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion2 && pCompanion2.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion3 && pCompanion3.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion4 && pCompanion4.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion5 && pCompanion5.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		endif
	endif
	...
	RegisterForHitEvent(pCompanion1)
	RegisterForHitEvent(pCompanion2)
	RegisterForHitEvent(pCompanion3)
	RegisterForHitEvent(pCompanion4)
	RegisterForHitEvent(pCompanion5)
...
Event Actor.OnCombatStateChanged(Actor RemoteSource, Actor akTarget, int aeCombatState)
	...
	if akTarget
		if RemoteSource == playerRef
			HandleCombatMessageToSituationAwareness(akTarget as Actor)
		else
			if (pCompanion1 && RemoteSource == pCompanion1.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion2 && RemoteSource == pCompanion2.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion3 && RemoteSource == pCompanion3.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion4 && RemoteSource == pCompanion4.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion5 && RemoteSource == pCompanion5.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			endif		
		endif
	endif
...
Event Actor.OnPlayerHealTeammate(Actor akSender, Actor akTeammate)
	...
	if akTeammate == DogmeatCompanion.GetActorReference()
		SendAffinityEvent(self, HealDogmeat)
	else	
		if (pCompanion1 && akTeammate == pCompanion1.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion1.GetActorReference(), None, true)
		elseif (pCompanion2 && akTeammate == pCompanion2.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion2.GetActorReference(), None, true)
		elseif (pCompanion3 && akTeammate == pCompanion3.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion3.GetActorReference(), None, true)
		elseif (pCompanion4 && akTeammate == pCompanion4.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion4.GetActorReference(), None, true)
		elseif (pCompanion5 && akTeammate == pCompanion5.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion5.GetActorReference(), None, true)
		endif
	endif
EndEvent
...
Event OnPlayerSleepStart(float afSleepStartTime, float afDesiredSleepEndTime, ObjectReference akBed)
	; Handled Directly by TweakDFScript
EndEvent
...
Event ReferenceAlias.OnCommandModeGiveCommand(ReferenceAlias akSender, int aeCommandType, ObjectReference akTarget)
	...
	if akSender == DogmeatCompanion && aeCommandType == 6
		...
		elseif (pCompanion2 && SenderActor == pCompanion2.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion3 && SenderActor == pCompanion3.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion4 && SenderActor == pCompanion4.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion5 && SenderActor == pCompanion5.GetActorReference())
			bFoundCommand = true
		endif
		if 	(bFoundCommand)
...
bool Function IsCompanion(Actor ActorToCheck)
	...
	return ActorToCheck.IsInFaction(CurrentCompanionFaction)
EndFunction

;EDITED BY AFT
bool Function IsPossibleCompanion(Actor ActorToCheck)
	;return ActorToCheck as CompanionActorScript
	return !ActorToCheck.IsInFaction(DisallowedCompanionFaction)
EndFunction
...
Function SetCompanion(Actor ActorToMakeCompanion, bool SetCompanion = true, bool FillCompanionAlias = true, bool SuppressDismissMessage = false)
	...
	trace(self, "SetCompanion() : SetCompanion = " + SetCompanion + "]")
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript

		; 1.14 : Use Async so scenes can continue 
		
		; bool gotSpinLock = GetSpinLock(pTweakMutexCompanions, 3, "SetCompanion")
		; DFScript.SetCompanion(ActorToMakeCompanion, SetCompanion, FillCompanionAlias, SuppressDismissMessage, gotSpinLock)
		; ReleaseSpinLock(pTweakMutexCompanions,gotSpinLock, "SetCompanion")

		; 1.14 bug: Not sure why, But Nick Valentine bugs out if we return to fast during his 
		; dismiss scene. So just Nick uses synchonous follow me. Everyone else gets 
		; async. 
		
		if ActorToMakeCompanion == (Game.GetForm(0x00002F24) as ActorBase).GetUniqueActor()
			; If Nick Valentine, Use Synchronous
			DFScript.SetCompanion(ActorToMakeCompanion, SetCompanion, FillCompanionAlias, SuppressDismissMessage)
		else
			; Otherwise use asynchronous
			Var[] params = new Var[5]
			params[0] = ActorToMakeCompanion
			params[1] = SetCompanion
			params[2] = FillCompanionAlias
			params[3] = SuppressDismissMessage
			params[4] = false
			DFScript.CallFunctionNoWait("SetCompanion", params)
		endif
		return
	else
...
Function DismissCompanion(Actor CompanionToDismiss, bool ShowLocationAssignmentListIfAvailable = true, bool SuppressDismissMessage = false)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.DismissCompanion(CompanionToDismiss, ShowLocationAssignmentListIfAvailable, SuppressDismissMessage)
		return
	else
		trace(self, "DismissCompanion - Cast to AFT:TweakDFScript failed")
	endif
...
Function DismissDogmeatCompanion(bool ShowLocationAssignmentListIfAvailable = true, bool SuppressDismissMessage = false)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.DismissDogmeatCompanion(ShowLocationAssignmentListIfAvailable, SuppressDismissMessage)
		return
	else
		trace(self, "DismissDogmeatCompanion - Cast to AFT:TweakDFScript failed")
	endif
...
function SetDogmeatCompanion(Actor DogmeatRef = None)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.SetDogmeatCompanion(DogmeatRef)
		return
	else
		trace(self, "SetDogmeatCompanion - Cast to AFT:TweakDFScript failed")
	endif
...
Function ClearCompanion()
	trace(self, "AFT Override : ClearCompanion (Ignored)")

	; The only place this is used in the original game is the COMCurieQuest. Thing is, right after
	; it is called, it calls Curie.DisallowCompanon(), which... calls dismissCompanion on Curie. 
	
	; So... we just ignore ClearCompanion for now to avoid the wrong person being removed. (Whoever
	; Companion1 is pointing at when you talk to Amari about Curie...)
	
	; TODO: Revisit once DLC is installed and confirm this is acceptable even with the DLC.
...
Function AllowCompanion(Actor ActorToAllow, bool MakeCompanionIfNoneCurrently = true, bool ForceCompanion = false)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.AllowCompanion(ActorToAllow, MakeCompanionIfNoneCurrently, ForceCompanion )
		return
	else
		trace(self, "AllowCompanion - Cast to AFT:TweakDFScript Failed")
	endif
...
; NOTE: This method is unused with AFT as SET/DISMISS are redirected to another
;       script which uses a local version of this method.
...
Function TryToTeleportCompanion(ObjectReference TeleportDestinationRef, bool ShouldPlayTeleportInEffect = true, bool ShouldPlayTeleportOutEffect = true)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.TryToTeleportCompanion(TeleportDestinationRef, ShouldPlayTeleportInEffect, ShouldPlayTeleportOutEffect )
		return
	else
		trace(self, "TryToTeleportCompanion - Cast to AFT:TweakDFScript Failed")
	endif
...
Function TryToTeleportDogmeat(ObjectReference TeleportDestinationRef, bool ShouldPlayTeleportInEffect = true, bool ShouldPlayTeleportOutEffect = true)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.TryToTeleportDogmeat(TeleportDestinationRef, ShouldPlayTeleportInEffect, ShouldPlayTeleportOutEffect )
		return
	else
		trace(self, "TryToTeleportDogmeat - Cast to AFT:TweakDFScript Failed")
	endif
...
; NOTE: Ignored with AFT as calling method redirected to another script which uses a 
;       local verison
...
Function FollowerWait(Actor Follower)
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.FollowerWait( Follower )
		return
	else
		trace(self, "FollowerWait - Cast to AFT:TweakDFScript failed")
	endif
...
Function FollowerFollow(Actor Follower)
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
		DFScript.FollowerFollow( Follower )
		return
	else
		trace(self, "FollowerFollow - Cast to AFT:TweakDFScript failed")
	endif
...
bool Function StartCompanionInteractionScene(GlobalVariable SceneTypeGlobal, objectReference SceneRef, location SceneLoc, Actor CompanionActor = None) global
	...
	If Game.IsPluginInstalled("AmazingFollowerTweaks.esp")
		Quest selfQuest = Game.GetFormFromFile(0x000289E4, "Fallout4.esm") as Quest	
		if selfQuest
		
			ReferenceAlias gv_Companion1 = (selfQuest.GetAlias(17) as ReferenceAlias)
			ReferenceAlias gv_Companion2 = (selfQuest.GetAlias(18) as ReferenceAlias)
			ReferenceAlias gv_Companion3 = (selfQuest.GetAlias(19) as ReferenceAlias)
			ReferenceAlias gv_Companion4 = (selfQuest.GetAlias(20) as ReferenceAlias)
			ReferenceAlias gv_Companion5 = (selfQuest.GetAlias(21) as ReferenceAlias)					
			Actor gv_CompanionActor1 = gv_Companion1.GetActorReference()
			Actor gv_CompanionActor2 = gv_Companion2.GetActorReference()
			Actor gv_CompanionActor3 = gv_Companion3.GetActorReference()
			Actor gv_CompanionActor4 = gv_Companion4.GetActorReference()
			Actor gv_CompanionActor5 = gv_Companion5.GetActorReference()

			bool ret = true
			bool trycompanion = true			
			if gv_CompanionActor1
				trycompanion = false
				trace(selfScript, "AFT StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor1 )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor1, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				Utility.wait(0.1)
			endif		
			if gv_CompanionActor2
				trycompanion = false
				trace(selfScript, "AFT : StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor2 )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor2, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				Utility.wait(0.1)
			endif
			if gv_CompanionActor3
				trycompanion = false
				trace(selfScript, "AFT : StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor3 )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor3, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				Utility.wait(0.1)
			endif
			if gv_CompanionActor4
				trycompanion = false
				trace(selfScript, "AFT : StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor4 )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor4, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				Utility.wait(0.1)
			endif
			if gv_CompanionActor5
				trycompanion = false
				trace(selfScript, "AFT : StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor5 )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor5, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
			endif
			if trycompanion
				Actor gv_CompanionActor = selfScript.Companion.GetActorReference()
				if gv_CompanionActor
					trace(selfScript, "AFT : Unexpected : Companion is filled but none of the aliases area.")
					trace(selfScript, "AFT : StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + gv_CompanionActor )
					RETURN selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = gv_CompanionActor, akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				endif				
			endif
			return ret
		else
			Trace(selfScript, "Unable to cast form [0x000289E4] to Quest Followers")
			; Fall through...
		endif
	endif
...
; AFT: LoiterRadius unused. Replaced by more efficient algorithm.
...
Function HandleTimer_LoiterSample()
	; Opt : No sqrts
	float FurthestDistanceSquared = GetFurthestDistanceAndPromoteSamples()
	traceconditional(self, "FurthestDistance = " + FurthestDistanceSquared, DebugTrace_Loitering)
...
float Function GetDistanceFromSamplePosition(int BufferIndex, float X, float Y, float Z)
	; NOTE : sqrt is exact, but expensive. And this method gets called 10 times a second... 
	;        When comparing lines for greater/less than, better to compare the squared 
	;        values... You only need sqrt if you are summing the distances for a total.  
...
bool Function LocalSendAffinityEvent(scriptobject Sender, keyword EventKeyword, objectReference Target = None, GlobalVariable EventSizeOverride = None, bool CheckCompanionProximity = true, bool ShouldSuppressComment = false, bool IsDialogueBump = false, float ResponseDelay = 1.0)
	trace(self, "FollowersScript: LocalSendAffinityEvent(" + EventKeyword + ", " + EventSizeOverride + ", " + CheckCompanionProximity + ") sender:" + sender)
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
	  return DFScript.LocalSendAffinityEvent(Sender, EventKeyword, Target, EventSizeOverride, CheckCompanionProximity, ShouldSuppressComment, IsDialogueBump, ResponseDelay)
	else
		trace(self, "WARN: TweakDFScript cast failure. Falling through to original code.")	
	endif

	if 	LockSendAffinityEvent
		trace(self, "FollowersScript: LocalSendAffinityEvent is true. Beginning Wait.")	
		...
		if 0 == maxwait
			trace(self, "Wait timed out. Proceeding")
		else
			trace(self, "LocalSendAffinityEvent now true.")
		endif
	else
		LockSendAffinityEvent = true
	endif
...
bool Function IsActorInLocationWhereMurderShouldBeIgnored(actor ActorToTest)
	...
	AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
	if DFScript
	  return DFScript.IsActorInLocationWhereMurderShouldBeIgnored(ActorToTest, LocationsToIgnoreMurderIn )
	endif
...
Function HandleOnCrippleEvent(Var akSender, ActorValue akActorValue, bool abCrippled) ;sent by Remotely Registered OnCripple events
	if abCrippled ;if we're crippled not healed
		if (akSender as Actor) == PlayerRef
			...
			ReferenceAlias akSenderRef = (akSender as ReferenceAlias)
			if akSenderRef
				if akSenderRef == pCompanion1
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion2
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion3
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion4
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion5
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				endif
			endif
		endif
...
actor Function GetNearbyInfatuatedRomanticCompanion()
	...
	; TODO : We will need additional checks to account for things that AFT might do like 
	; place the NPC in a POSE scene. If the NPC is sleeping, may need to undo that 
	; setting so they can wake up with player, etc... 
...
Function FlagCompanionChatEvent(actorvalue ActorValueRepresentingEvent) global
	...
	; NOTE: Can't redefine global functions within a state override, but we can make
	; new functions (with suffix like Override) and call them.
	
	FollowersScript selfScript = GetScript()
	If Game.IsPluginInstalled("AmazingFollowerTweaks.esp")
		Quest selfQuest = Game.GetFormFromFile(0x000289E4, "Fallout4.esm") as Quest	
		if selfQuest
			AFT:TweakDFScript.FlagCompanionChatEventOverride(ActorValueRepresentingEvent)
			return
		else
			Trace(selfScript, "AFT: FollowersScript.FlagCompanionChatEvent() - Unable to cast form [0x000289E4] to Quest Followers")
		endif
	endif
	...
	if CompanionActor && CompanionActor.IsPlayerNearby()
		debug.trace("AFT: FollowersScript.FlagCompanionChatEvent() - CompanionActor is nearby, setting ActorValue to 1: " + ActorValueRepresentingEvent + ", " + CompanionActor)
...
Function SetSAECombatant(keyword LocEncKeyword, scriptObject SettingObject)
	...
	if FoundEncDefinition == false
		...
		if resetsaecombatant
			resetsaecombatant = false
			return ; AFT VANILLA BUG FIX?
		endif
...
Function SetAutonomyPrivate(ScriptObject CallingObject, bool AutonomousActivityAllowed = True)
	...
	if AutonomyDisallowedByObjects.length == 0
		...
		; No need to re-send for each companion as the handler on AOAliasBailoutScript doesn't check the args anyway. 
		; It just bails on whatever AO scene is playing.
...
ReferenceAlias pCompanion1
ReferenceAlias pCompanion2
ReferenceAlias pCompanion3
ReferenceAlias pCompanion4
ReferenceAlias pCompanion5
GlobalVariable pTweakMutexCompanions
bool needs_initialize = true

; Vanilla Bug Fixes (typically created by activating god mode during combat)
bool resetsas = false
bool resetsaeclutter = false
bool resetsaecombatant = false

; Deprecated, but left in to avoid upgrade errors
bool acceptOnHit ;Deprecated 

;*******************************************************************************************************
;*************************************     Aft Helper Methods   **********************************************
;*******************************************************************************************************

; attempt = -1 means wait forever... use sparingly
;bool Function GetSpinLock(GlobalVariable mutex, int attempts = 0, string sourcehint = "")
;	if (1.0 == mutex.mod(1))
;		trace(self, "Giving SpinLock to [" + sourcehint + "]")
;		return true
;	endif
;	while (attempts != 0)
;		Utility.wait(0.1)
;		if (0.0 == mutex.GetValue() && 1.0 == mutex.mod(1))
;			trace(self, "Giving SpinLock to [" + sourcehint + "] (" + attempts + " attempts left)")
;			return true
;		endif
;		attempts -= 1
;	endwhile
;	trace(self, "[" + sourcehint + "] Failed to get SpinLock")
;	return false
;endFunction

;Function ReleaseSpinLock(GlobalVariable mutex, bool gotLock = true, string sourcehint = "")
;	if gotLock
;		trace(self, "[" + sourcehint + "] Releasing SpinLock")
;		mutex.SetValue(0.0)
;	else
;		trace(self, "Ignoring Spinlock Release Request from [" + sourcehint + "]")		
;	endif
;endFunction

Function ReleaseLockSendAffinityEvent()
	trace(self, "ReleaseLockSendAffinityEvent()")
	LockSendAffinityEvent = false
EndFunction
...
Function initialize()
	if needs_initialize
		...
		pCompanion1           = ((self as Quest).GetAlias(17) as ReferenceAlias)
		pCompanion2           = ((self as Quest).GetAlias(18) as ReferenceAlias)
		pCompanion3           = ((self as Quest).GetAlias(19) as ReferenceAlias)
		pCompanion4           = ((self as Quest).GetAlias(20) as ReferenceAlias)
		pCompanion5           = ((self as Quest).GetAlias(21) as ReferenceAlias)
		pTweakMutexCompanions = (Game.GetFormFromFile(0x01000F9C, "AmazingFollowerTweaks.esp") as GlobalVariable)
		...
		RegisterEvents()
		trace(self, "initialize() finished.")
	else
		trace(self, "initialize() skipped")		
	endif	
EndFunction
...
Function RegisterEvents()
	...
	; TODO : Companion Events should move to alias scripts	
	RegisterForRemoteEvent(pCompanion1, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion2, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion3, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion4, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion5, "OnCripple") ;used by the Affinity Event system
	
	RegisterForRemoteEvent(pCompanion1, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion2, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion3, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion4, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion5, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	
	RegisterForHitEvent(playerRef)    ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion1)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion2)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion3)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion4)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion5)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	
EndFunction

; Fired By Quest TweakMonitorPlayer
Function OnGameLoaded(bool firstTime=false)
	trace(self, "Aft: OnGameLoaded() Called")
	trace(self, "Aft: Current State of FollowersScript is [" + GetState() + "]")

	; Fix infinite loops caused by users using TGM during combat....
	resetsas          = true
	resetsaeclutter   = true
	resetsaecombatant = true
	...
	cancelTimer(iTimerID_LoiterSample)
	startTimer(TimerInterval_LoiterSample, iTimerID_LoiterSample)
		
	trace(self, "Aft: OnGameLoaded() finished.")
endFunction

Function AftReset()
	trace(self, "============= AftReset() ================")
		
	UnRegisterForRemoteEvent(pCompanion1, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion2, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion3, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion4, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion5, "OnCripple") ;used by the Affinity Event system
	
	UnRegisterForRemoteEvent(pCompanion1, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion2, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion3, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion4, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion5, "OnCommandModeGiveCommand") ;used to start quests for specific behavior	
...
; Expose state so it can be controlled externally
