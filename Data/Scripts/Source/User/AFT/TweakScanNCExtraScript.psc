Scriptname AFT:TweakScanNCExtraScript extends Quest

; EXTRA items are special as they produce no scrap, but we still remove/record them
; for settlement build reasons. 

; import AFT

Quest			Property TweakScrapScanMaster		Auto Const
FormList		Property TweakNonConstructed_Extra	Auto Const
...
bool Function Trace(string asTextToPrint, int aiSeverity = 0) debugOnly
	string logName = "TweakScanNCExtraScript"
...
Function ScanHelper()
	...
	ObjectReference[] results = center.FindAllReferencesOfType(TweakNonConstructed_Extra, radius)			
	...
	while (i != numresults && keepgoing)
		...
		elseif (!result.IsDisabled())
			...
			if snapshot		
				...
				params[9] = -1
				Trace("Adding Components [Extra] to scrapdata")
				ScrapScanMaster.CallFunctionNoWait("TweakBuildInfo", params)			
			else			
				result.SetPosition(0,0,0)
				result.Disable()
				result.Delete()
				Trace("Adding Scrap [Extra] to scrapdata")
			endif			
		endif	
